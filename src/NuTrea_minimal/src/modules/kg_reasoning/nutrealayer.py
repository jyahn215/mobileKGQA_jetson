...
class NuTreaLayer(BaseGNNLayer):
    ...
    def init_reason(self, local_entity, kb_adj_mat, local_entity_emb, rel_features, rel_features_inv, query_entities, init_dist):
    ...
    def reason_layer(self, curr_dist, instruction, rel_linear, pos_emb, inverse=False):
        ...
        rel_features = self.rel_features if inverse is False else self.rel_features_inv
        ...
        if pos_emb is not None:
            ...
            fact_val = F.relu(rel_linear(fact_rel) * fact_query) # default setting: pos_emb = None
    ...
    def forward(self, current_dist, relational_ins, relational_con, step=0, return_score=False):
        ...
        for j in range(relational_ins.size(1)):
            ...
            neighbor_rep = self.reason_layer(
                current_dist, relational_ins[:,j,:], rel_linear, pos_emb) # B x 2000 x D
            neighbor_reps.append(neighbor_rep)

            neighbor_rep = self.reason_layer(
                current_dist, relational_ins[:,j,:], rel_linear, pos_emb_inv, inverse=True)
            neighbor_reps.append(neighbor_rep)
        ...
        for j in range(relational_con.size(1)):
            pooled_rep = self.pool_subgraph(
                self.leaf_nodes, relational_con[:,j,:], con_linear, depth=self.backup_depth)
            subgraph_reps.append(pooled_rep)

            pooled_rep = self.pool_subgraph(
                self.leaf_nodes, relational_con[:,j,:], con_linear, depth=self.backup_depth, inverse=True)
            subgraph_reps.append(pooled_rep)
