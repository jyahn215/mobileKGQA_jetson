    ...
    def __init__(self, config, word2id, relation2id, entity2id, tokenize, data_type="train"):
        ...
        if config["bit"] == None:
            self.hidden_path = f"./data/preprocessed_data/{self.dataset_name}/last_hidden_state/" \
                        f"{self.tokenize}/queries/{self.domain}/{self.data_type}"
        else:
            bit = config["bit"]
            self.hidden_path = f"./data/preprocessed_data/{self.dataset_name}/last_hidden_state/" \
...
def load_data(config, tokenize):
    ...
    rel_hidden = torch.load(os.path.join(config['data_folder'], "./last_hidden_state/",
                                            config["lm"], "rel", "rel_all.pt"), map_location='cpu')
    rel_hidden.requires_grad = False
    rel_mask = torch.load(os.path.join(config['data_folder'], "./last_hidden_state/",
                                            config["lm"], "rel", "rel_mask.pt"), map_location='cpu')
    rel_mask.requires_grad = False
    rel_pad = torch.load(os.path.join(config['data_folder'], "./last_hidden_state/",
                                            config["lm"], "rel", "rel_pad.pt"), map_location='cpu')
    rel_pad.requires_grad = False
    rel_pad_mask = torch.load(os.path.join(config['data_folder'], "./last_hidden_state/",
                                            config["lm"], "rel", "rel_pad_mask.pt"), map_location='cpu')
    ...
    rel_inv_hidden = torch.load(os.path.join(config['data_folder'], "./last_hidden_state/",
                                            config["lm"], "rel_inv", "rel_inv_all.pt"), map_location='cpu')
    rel_inv_hidden.requires_grad = False
    rel_inv_mask = torch.load(os.path.join(config['data_folder'], "./last_hidden_state/",
                                            config["lm"], "rel_inv", "rel_inv_mask.pt"), map_location='cpu')
    rel_inv_mask.requires_grad = False
    rel_inv_pad = torch.load(os.path.join(config['data_folder'], "./last_hidden_state/",
                                            config["lm"], "rel_inv", "rel_inv_pad.pt"), map_location='cpu')
    rel_inv_pad.requires_grad = False
    rel_inv_pad_mask = torch.load(os.path.join(config['data_folder'], "./last_hidden_state/",
                                            config["lm"], "rel_inv", "rel_inv_pad_mask.pt"), map_location='cpu')
    ...
    if config["debug"] :
        ...
        train_data = None
        # train_path = os.path.join(train_config['data_folder'], "train.json")
        # print(f"train_data is from {train_path}")
        ...
        test_path = os.path.join(test_config['data_folder'], "test.json")
        print(f"test_data is from {test_path}")
