...
class NuTrea(BaseModel):
    def __init__(self, args, num_entity, num_relation, rel_hidden, rel_mask, rel_inv_hidden, rel_inv_mask):
        """
        Init NuTrea model.
        """
        super(NuTrea, self).__init__(args, num_entity, num_relation)
        self.args = args
        ...
        self.IEF = nn.Parameter(torch.zeros(13300), requires_grad=False)
        #self.IEF = nn.Parameter(torch.zeros((num_relation+1)*2), requires_grad=False)
        self.emb_cache = {}

        self.rel_hidden = rel_hidden
        self.rel_mask = rel_mask.float()
        self.rel_inv_hidden = rel_inv_hidden
        self.rel_inv_mask = rel_inv_mask.float()
    ...
    def get_rel_feature(self):
        rel_features = self.instruction.question_emb(self.rel_hidden)
        rel_features_inv = self.instruction.question_emb(self.rel_inv_hidden)
        rel_features = self.self_att_r(rel_features, self.rel_mask)
        rel_features_inv = self.self_att_r(rel_features_inv, self.rel_inv_mask)
    ...
    def private_module_def(self, args, num_entity, num_relation):
        ...
        if args['lm'] == 'lstm':
            ...
            self.constraint = BERTInstruction(args, args['lm'], constraint=True)
    ...
    def init_reason(self, curr_dist, local_entity, kb_adj_mat, query_entities):
        """
        Initializing Reasoning
        """
        # batch_size = local_entity.size(0)
        self.local_entity = local_entity
        # self.instruction(query_hidden, query_mask) # self.instruction is BERTInstruction object
        # self.constraint(query_hidden, query_mask) # self.constraint is BERTInstruction object
    ...
    def train_ief(self, batch):
        local_entity, query_entities, kb_adj_mat, query_hidden, query_mask, seed_dist, answer_dist = batch
        ...
        query_hidden = query_hidden.to(self.device) if query_hidden.device != self.device else query_hidden
        query_mask = query_mask.to(self.device) if query_mask.device != self.device else query_mask

        self.init_reason(curr_dist=current_dist, local_entity=local_entity, kb_adj_mat=kb_adj_mat, query_entities=query_entities)
        
        
        bsize, bnode_num = answer_dist.shape
        node_num = bsize * bnode_num
        rtype_num = self.rel_hidden.size(0)
    ...
    def forward(self, batch, training=False):
        ...
        query_mask = query_mask.to(self.device) if query_mask.device != self.device else query_mask
        # end = time.time()
        # print('data loading time:', end-st)
        ...
        #st = time.time()
        self.init_reason(curr_dist=current_dist, local_entity=local_entity, kb_adj_mat=kb_adj_mat, query_entities=query_entities)
        # self.reasoning(NuTreaLayer)를 초기화
        
        self.instruction.init_reason(query_hidden, query_mask)
        for i in range(self.num_expansion_ins):
            relational_ins = self.instruction.get_instruction(self.instruction.relational_ins, step=i) 
            self.instruction.instructions.append(relational_ins.unsqueeze(1))
            self.instruction.relational_ins = relational_ins

        self.constraint.init_reason(query_hidden, query_mask)
        for i in range(self.num_backup_ins):
            relational_ins = self.constraint.get_instruction(self.constraint.relational_ins, step=i) 
        ...
        # end = time.time()
        # print('instruction generation time:', end-st)
        ...
        for t in range(self.num_iter):
            # st = time.time()
            relation_ins = torch.cat(self.instruction.instructions, dim=1)   # (batch_size, num_expansion_ins, entity_dim)
            relation_con = torch.cat(self.constraint.instructions, dim=1)  # (batch_size, num_backup_ins, entity_dim)
            ...
            for j in range(self.num_layers):
                ...
                raw_score, self.curr_dist, global_rep, gs, hs = \
                    self.reasoning(self.curr_dist, relation_ins, relation_con, step=j, return_score=True)
            self.dist_history.append(self.curr_dist)
            self.score_history.append(raw_score)
            # end = time.time()
            # print('reasoning time:', end-st)
            ...
            # end = time.time()
            # print('expansion instruction update time:', end-st)
            ...
            # end = time.time()
            # print('backup instruction update time:', end-st)
