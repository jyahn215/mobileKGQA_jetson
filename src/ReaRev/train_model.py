...
class Trainer_KBQA(object):
    ...
    def train(self, start_epoch, end_epoch):
        ...
        for epoch in range(start_epoch, end_epoch + 1):
            ...
            if (epoch + 1) % eval_every == 0:
                eval_f1, eval_h1, eval_em = self.evaluate(self.eval_data, self.test_batch_size)
                self.logger.info("EVAL F1: {:.4f}, H1: {:.4f}, EM {:.4f}".format(eval_f1, eval_h1, eval_em))
                wandb.log({"Epoch": epoch, "eval_f1": eval_f1, "eval_h1": eval_h1, "eval_em": eval_em})
                ...
                if epoch > self.warmup_epoch:
                    if eval_h1 > self.best_h1:
                        ...
                        wandb.log({"Epoch": epoch, "best_h1": self.best_h1})
                        do_test = True
                    if eval_f1 > self.best_f1:
                        self.best_f1 = eval_f1
                        self.save_ckpt("f1")
                        self.logger.info("BEST EVAL F1: {:.4f}".format(eval_f1))
                        wandb.log({"Epoch": epoch, "best_f1": self.best_f1})
                        do_test = True

                test_f1, test_h1, test_em = self.evaluate(self.test_data, self.test_batch_size)
                ...
                if do_test:
                    wandb.log({"Epoch": epoch, "best_test_f1": test_f1, "best_test_h1": test_h1})
                # if do_test:
                #     eval_f1, eval_h1 = self.evaluate(self.test_data, self.test_batch_size)
                #     self.logger.info("TEST F1: {:.4f}, H1: {:.4f}".format(eval_f1, eval_h1))
    ...
    def evaluate_best(self):
        ...
        self.logger.info("TEST F1: {:.4f}, H1: {:.4f}, EM {:.4f}".format(eval_f1, eval_h1, eval_em))
        wandb.log({"final_test_f1": eval_f1, "final_test_h1": eval_h1, "final_test_em": eval_em})
