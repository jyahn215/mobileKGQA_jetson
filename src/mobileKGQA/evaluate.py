...
class Evaluator:
    ...
    def evaluate(self, valid_data, test_batch_size=20, write_info=False, test_domain="total"):
        # write_info = True
        self.model.eval()
        self.count = 0
        eps = self.eps
        ...
        if write_info and self.file_write is None:
            ...
            filename = os.path.join("./ckpts/mobileKGQA",
                                    "{}_test_{}.info".format(filename, test_domain))
            print("write info to file: ", filename)
        ...
        for iteration in tqdm(range(num_epoch)):
            ...
            batch, batch_qids = batch[:-1], batch[-1]
            with torch.no_grad():
            ...
            for batch_id in range(batch_size):
                ...
                if write_info:
                    tp_obj = dict()
                    tp_obj["qid"] = str(batch_qids[batch_id])
                    tp_obj['answers'] = ans
