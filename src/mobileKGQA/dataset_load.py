...
class BasicDataLoader(object):
    ...
    def __init__(self, config, word2id, relation2id, entity2id, tokenize, data_type="train"):
        ...
        self.tokenize = config["lm"]
        self.dataset_name = config["dataset_name"]
        if data_type == "train" or data_type == "dev":
            # if "+" not in config["domain"]: 
            self.domain = config["domain"]
            if config["bit"] == None:
                self.hidden_path = f"./data/preprocessed_data/{self.dataset_name}/last_hidden_state/" \
                            f"{self.tokenize}/ori/queries/{self.domain}/{self.data_type}"
            else:
                bit = config["bit"]
                self.hidden_path = f"./data/preprocessed_data/{self.dataset_name}/last_hidden_state/" \
                            f"{self.tokenize}/{bit}bit/queries/{self.domain}/{self.data_type}"

            # else:
            #     merged = config["domain"]
            #     with open(f"./data/preprocessed_data/{self.dataset_name}/{merged}/domain_meta_data.pkl", "rb") as rf:
            #         domain_meta_data = pickle.load(rf)
            #     domain_num_list = [domain_meta_data[f"{domain}_{self.data_type}"] for domain in config["domain"].split("+")]
            #     self.domain = config["domain"]
            #     self.domain_name_list = config["domain"].split("+")
            #     assert domain_num_list is not None
            #     self.domain_num_list = []
            #     accum = domain_num_list[0]
            #     for domain_num in domain_num_list[1:]:
            #         self.domain_num_list.append(accum)
            #         accum += domain_num
            #     self.domain_num_list.append(accum)
            #     if config["bit"] == None:
            #         self.hidden_path = f"./data/preprocessed_data/{self.dataset_name}/last_hidden_state/{self.tokenize}/ori/queries"
            #     else:
            #         bit = config["bit"]
            #         self.hidden_path = f"./data/preprocessed_data/{self.dataset_name}/last_hidden_state/{self.tokenize}/{bit}bit/queries"
            
        elif data_type == "test":
            self.domain = config["test_domain"]
            if config["bit"] == None:
                self.hidden_path = f"./data/preprocessed_data/{self.dataset_name}/last_hidden_state/" \
                            f"{self.tokenize}/ori/queries/{self.domain}/{self.data_type}"
            else:
    ...
    def _load_data(self):
        ...
        self._prepare_data()
        self.question_id = np.array(self.question_id, dtype=str)
    ...
    def _prepare_data(self):
        ...
        """
        # self.max_query_word (16) for webqsp, (32) for CWQ
        # in gemma max_query_word (19) for webqsp, (44) for CWQ
        ...
        # else:
        #     if self.tokenize == "qwen2:0.5b":
        #         tokenizer_name = "PrunaAI/Qwen-Qwen2-0.5B-Instruct-bnb-4bit-smashed"
        #     elif self.tokenize == "gemma2:2b":
        #         tokenizer_name = "google/gemma-2-2b-it"
        #     elif self.tokenize == "phi3.5":
        #         tokenizer_name = "unsloth/Phi-3.5-mini-instruct-bnb-4bit"
        #     elif self.tokenize == "llama3.1":
        ...
        #     # elif self.tokenize  == 't5':
        #     #     tokenizer_name = 't5-small'
        #     # if self.tokenize  == 'relbert':
        #     #     tokenizer_name = './ckpts/ReaRev/pretrained_lms/sr-simbert/'

        #     self.max_query_word = max_count + 2 #for cls token and sep
        #     #self.tokenizer = AutoTokenizer(self.max_query_word)
        #     self.tokenizer = AutoTokenizer.from_pretrained(tokenizer_name, cache_dir="./ckpts/llm")
        ...
        for sample in tqdm(self.data):
            ...
            for answer in sample['answers']:
                ...
                if text_answer in self.entity2id:
                    answer_ent = self.entity2id[text_answer]
                    answer_list.append(answer_ent)
                    if answer_ent in g2l:
                        self.answer_dists[next_id, g2l[answer_ent]] = 1.0
    ...
    #         tokenizer = AutoTokenizer.from_pretrained(tokenizer_name, cache_dir="./ckpts/llm")
    ...
    def load_query_last_hidden(self, sample_ids):
        
        # if "+" not in self.domain:
        hidden_list = [torch.load(os.path.join(self.hidden_path, f"{idx}.pt"), map_location="cpu") for idx in sample_ids]
        for idx in range(len(hidden_list)):
            hidden_list[idx].requires_grad = False
        hidden_list = torch.stack(hidden_list)
        hidden_list.requires_grad = False
        hidden_list = hidden_list.to(torch.float32) if hidden_list.dtype != torch.float32 else hidden_list

        mask_list = [torch.load(os.path.join(self.hidden_path, f"{idx}_mask.pt"), map_location="cpu") for idx in sample_ids]
        for idx in range(len(mask_list)):
            mask_list[idx].requires_grad = False
        mask_list = torch.stack(mask_list)
        mask_list.requires_grad = False
        ...
        #         cnt = 0
        #         for domain_num in self.domain_num_list:
        #             if idx >= domain_num:
        #                 cnt += 1
        #         if cnt == 0:
        #             adj = 0
        #         else:
        #             adj = self.domain_num_list[cnt-1]
        #         domain_name = self.domain_name_list[cnt]
        #         hidden_path = os.path.join(self.hidden_path, domain_name, self.data_type)
        #         hidden_list.append(torch.load(os.path.join(hidden_path, f"{idx - adj}.pt"), map_location="cpu"))
        #         mask_list.append(torch.load(os.path.join(hidden_path, f"{idx - adj}_mask.pt"), map_location="cpu"))
...
class SingleDataLoader(BasicDataLoader):
    ...
    def get_batch(self, iteration, batch_size, fact_dropout, q_type=None, test=False):
        ...
        query_hidden, query_mask = self.load_query_last_hidden(sample_ids)
