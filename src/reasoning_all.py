...
def read_prediction(args):
    ...
    with open(f"./ckpts/mobileKGQA/{args.config}_test_{args.domain}.info", "rb") as rf:
        cands_list = [json.loads(line)["cand"] for line in tqdm(rf.readlines())]

    return idx2qinfo, qinfo2idx, cands_list


def LLM_RAG(args, idx2qinfo, qinfo2idx, cands_list):
    model = OLLAMA_LLM(
    ...
    with open(f"./data/preprocessed_data/{args.dataset}/_domains/{args.domain}/RoG-{args.dataset}_{args.domain}_test.json", "rt") as rf:
        test_dataset = [json.loads(line) for line in tqdm(rf.readlines())]

    f1_list = []
    hit_list = []
    wf = open(f"logs_{args.config}_{args.model_name}.txt", "wt")
    for idx, _cands in enumerate(tqdm(cands_list)):
        qid = test_dataset[idx]["id"]
        split, _idx = qinfo2idx[qid]
        question_entities = test_dataset[idx]["q_entity"]
        answer_entities = test_dataset[idx]["a_entity"]
        ...
        if args.shortest_path:
            ...
            print(f"hit: {sum(hit_list) / len(hit_list)}, F1: {sum(f1_list) / len(f1_list)}, config: {args.config}, model: {args.model_name}")
            print("----------------------------------------------------------------")
        else:
            plan_dict = dict()
            for cand, score in tqdm(_cands):
                for q_entity in question_entities:
                    try:
                        paths = list(nx.all_simple_paths(nx_graph, source=q_entity, target=cand, cutoff=args.max_hop))
                    except:
                        continue
                    ...
                    for path in paths:
                        ...
                        for idx in range(len(path)-1):
                            plan.append(nx_graph[path[idx]][path[idx+1]]["relation"])
                        if tuple(plan) not in plan_dict:
                            path_as_text = []
                            for idx in range(len(plan)*2):
                                if idx % 2 == 0:
                                    path_as_text.append(path[idx//2])
                                else:
                                    path_as_text.append(plan[idx//2])
                            path_as_text.append(path[-1])
                            plan_dict[tuple(plan)] = " -> ".join(path_as_text)

            plan_list = list(plan_dict.keys())
            path_as_text_list = list(plan_dict.values())

            cnt = 0
            bulk_input = ""
            for path_as_text in path_as_text_list:
                cnt += 1
                bulk_input += f"{cnt}. {path_as_text}\n"

            plan_num = min(args.plan_num, len(path_as_text_list))
            plan_selection_input = plan_selection_prompt.format(
                question=question, plan_candidates=bulk_input, plan_num=plan_num)
                
            plan_selection_message = [{"role": "user", "content": plan_selection_input}]  
            response = model.generate_chat_response(plan_selection_message)
            response = response.content

            plan_idx_list = response.split(",")
            plan_idx_list = [plan_idx.strip() for plan_idx in plan_idx_list]
            plan_idx_list_numeric = []
            for plan_idx in plan_idx_list:
                try:
                    plan_idx_list_numeric.append(int(plan_idx)-1)
                except:
                    pass
            plan_idx_list = plan_idx_list_numeric

            selected_plan_list = []
            for plan_idx in plan_idx_list:
                if plan_idx < len(plan_list):
                    selected_plan_list.append(plan_list[plan_idx])

            reasoning_path_list = []
            for question_entity in question_entities:
                for selected_plan in selected_plan_list:
                    reasoning_path_list += find_all_relation_paths(nx_graph, question_entity, list(selected_plan))
            ...
            print(f"hit: {sum(hit_list) / len(hit_list)}")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--bulk_thr", type=int, default=10)
    ...
    idx2qinfo, qinfo2idx, cands_list = read_prediction(args)
    answers = LLM_RAG(args, idx2qinfo, qinfo2idx, cands_list)
